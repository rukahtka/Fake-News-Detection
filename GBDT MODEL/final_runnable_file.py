# -*- coding: utf-8 -*-
"""
Created on Fri Feb  1 23:04:37 2019
...
def train():
    ...
    pred_prob_y = bst.predict(dtest).reshape(test_x.shape[0], 2) # predicted probabilities
    ...
    stances = pd.read_csv("gdbt_testing_input.csv", encoding = 'cp1252') # same row order as predicted
    stance_original = pd.read_csv("gdbt_testing_ouput.csv", encoding = 'cp1252')
    
    from sklearn.metrics import confusion_matrix
    cm = confusion_matrix(stance_original, pred_y)
    print(cm)
    
    
    print(pred_y.shape)
    print(stance_original.shape)
    df_output = pd.DataFrame()
    df_output['Headline'] = stances['Headline']
    df_output['Body'] = stances['articleBody']
    df_output['Actual Stance'] = stance_original
...
def cv():
    ...
    holdout_ids = set([int(x.rstrip()) for x in open('hold_out_ids.txt')])
    ...
    for fold, (trainInd, validInd) in enumerate(kf.split(cv_x, cv_y, groups)):
        ...
        print ('fold ',fold,' , score = ',s,' , perfect_score ', s_pref,' , weighted percentage ',wscore)
        scores.append(s)
